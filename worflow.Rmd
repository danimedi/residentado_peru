...
# select and filter the useful values
...
dat_final <- dat_joined %>% 
  select(
    ...
    `Especialidad/SubEspecialidad_ingresantes`, V9,
    V11_postulantes, V11_ingresantes, V13,
...
```{r}
i <- !dat_final$apellido_paterno %in% applicants$`Apellido Paterno`
phantom_dat <- dat_final[i, ]

# reveal identity and join data
x <- incomings[incomings$`No Doc.` %in% phantom_dat$num_doc, ]
left_join(x, applicants, by = c("year", "Apellido Paterno", "Apellido Materno",
                                "Nombres"))

# INCLUDE THIS DATA IN THE FINAL DATA AND REMOVE THE PREVIOUS DATA OF THESE PEOPLE INCLUDED PREVIOUSLY
```
